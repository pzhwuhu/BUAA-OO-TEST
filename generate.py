...
read_prob = float(config['read_prob'])
return_prob = float(config['return_prob'])
restore_prob = float(config['restore_prob'])
...
class data_generator:
    ...
    def generate_basic_commands(self):
        """
        生成除图书初始化外的其他基本交互指令：
        命令类型包括查询、借书、借阅书籍和预约（预约后后续会生成取书指令）。
        ...
        for i in range(t_command_num):
            ...
            if prob < query_prob:
                ...
                tmp_commands.append(self.generate_read())
        # 将所有命令随机划分成与日期数量相同的组
    ...
    def generate_read(self)->str:
        pid=random.choice(list(self.library.persons.keys()))
        isbn=random.choice(list(self.library.bs.keys()))
        return f"{pid} read {isbn}\n"

    def generate_restored(self, person_id, book_id)->str:
        """
        生成还书命令：
        格式为 "<personId> restored <BookId>\n"
        """
        return f"{person_id} restored {book_id}\n"
    ...
    def add_command(self, output=""):
        ...
        if 'accept' in output:
            ...
            if ('borrowed' in output or "picked" in output) and random.random() < return_prob:
                delta = end_date - date
            ...
            if 'ordered' in output:
                ...
                if prob < pick_2_prob:  # 生成两条取书命令
                    ...
                    if next_date_str == date_str:
                        ...
                        self.date2commands[next_date_str] = [command]
                        self.date2commands = dict(sorted(self.date2commands.items()))
            # 对于借阅成功的书籍，随机生成一条还书命令
            if 'read' in output and random.random() < restore_prob:
                # 当天归还 
                command = self.generate_restored(person_id, book_id)
                insert_place = random.randint(self.index, len(self.date2commands[date_str]))
                self.date2commands[date_str].insert(insert_place, command)
