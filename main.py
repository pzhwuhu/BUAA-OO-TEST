...
def process_function(case_id) -> str:
    ...
    try:
        ...
        while True:
            ...
            if output_command[0] == '':
                error_message = []
                error_message.append(proc.stderr.readline())
                while error_message[-1] != '':
                    error_message.append(proc.stderr.readline())
            ...
            if not('-' in output_command[0] or int(output_command[0])==0):
                for i in range(0,int(output_command[0])):
                    output_command.append(proc.stdout.readline())
            if 'trace' in output_command[0]: # moving trace
                moving_trace_value = None
                # 匹配格式: [YYYY-mm-dd] <类别号-序列号-副本号> moving trace: <转运次数>
                m = re.match(r'\[(\d{4}-\d{2}-\d{2})\]\s+([ABC]-\d{4}(?:-\d{2})?)\s+moving trace:\s*(\d+)', output_command[0])
                if m and int(m.group(3)) != 0:
                        moving_trace_value = m.group(3)
                        for i in range(0, int(moving_trace_value)):
                            output_command.append(proc.stdout.readline())
            ...
            if 'OPEN' in input_command:
                ...
                if int(output_command[0])>0:
                    for i in range(1,output_command.__len__()):
                        ...
                        if result!='':
                            return result
            elif 'queried' in input_command:
                result=library.query(input_command,output_command)
                if result != '':
                    return result
            else:
