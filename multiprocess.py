...
def run(jar_files, interact):
    ...
    while True:
        test_case += 1
        input_str = generate.generate()
        f = open(r"input\input" + str(test_case) + ".txt", "w")
        f.write(input_str)
        f.close()
        if (interact):
            print("---->   epoch " + str(test_case) + "   ---   case:" + str(test_case) + "   ---   wrong: " + str(wrong) + "   ---   tle: " + str(tle) + "   <----")
            print(str(input_str.count('\n')) + " lines")

        ans = ""
        try:
            ans = run_java.execute_java(input_str, 'm.jar')[0].replace('\r', '')
        except subprocess.TimeoutExpired as e:
            tle = tle + 1
            continue
        except Exception as e:
