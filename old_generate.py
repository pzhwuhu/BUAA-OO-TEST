...
def generate(instr_num=3000, people_num=500, tag_num=500, account_num=200, article_num=1000):
    ...
    mode_probabilities = {
        ...
        'special_hack_delay_rebuild': 0.3         
    }
    ...
    for mode, prob in mode_probabilities.items():
        ...
        if k < cumulative_prob:
            ...
            elif mode == 'special_hack_delay_rebuild':
...
def stress_test_data(instr_num=3000, people_num=100, tag_num=100, account_num=100, article_num=5000):
    """生成压力测试数据"""
...
def structured_social_media_test(instr_num=3000, people_num=100, tag_num=100, account_num=100, article_num=5000):
    """生成有逻辑、梯度的公众号测试数据"""
    instrlist = []
    created_accounts = set()
    created_articles = set()
...
def special_qtvs(instr_num=3000, people_num=2000):
    ...
    # === 阶段1：高频关系操作（ar/mr）===
    phase1_size = instr_num // 4
    for _ in range(phase1_size):
        # 随机选择ar或mr操作
        if random() < 0.3:  # 30%概率修改现有关系
            p1, p2 = 1, randint(1, people_num)  # 修改与1号的关系
    ...
    # === 阶段2：海量qtvs查询（前半）===
    phase2_size = instr_num // 4
    for _ in range(phase2_size):
        instrlist.append("qtvs 1 1")  # 集中测试tag_id=1
    
    # === 阶段3：混合操作（ar/mr/dft）===
    phase3_size = instr_num // 4
    for _ in range(phase3_size):
        op_type = random()
        if op_type < 0.5:  # 50%概率修改关系
            p1, p2 = randint(1, 10), randint(1, people_num)
            instrlist.append(f"mr {p1} {p2} {randint(int(-MAX_VALUE / 2), MAX_VALUE)}")
        elif op_type < 0.8:  # 30%概率新增关系
            p1, p2 = sorted([randint(1, people_num), randint(1, people_num)])
            if p1 != p2:
                instrlist.append(f"ar {p1} {p2} {randint(1, MAX_VALUE)}")
        else:  # 20%概率从标签中删除
            target = randint(1, people_num)
            instrlist.append(f"dft {target} 1 1")  # 删除target的标签1
    
    # === 阶段4：海量qtvs查询（后半）===
    phase4_size = instr_num - len(instrlist)  # 剩余指令
    for _ in range(phase4_size):
        instrlist.append("qtvs 1 1")
    
    return '\n'.join(instrlist[:instr_num])  # 确保总指令数准确


def special_qba_stress_test(instr_num=3000, people_num=100, max_relation_value=200):
    """高频qba压力测试生成器(动态关系网络+密集查询）"""
    instrlist = []
    
    # 创建核心用户群（集中在前20%的ID）
    core_users = int(people_num * 0.2)
    for i in range(1, core_users + 1):
        instrlist.append(f"ap {i} CoreUser_{i} {randint(1, 100)}")
    
    # 建立核心用户间的强关系网络
    for i in range(1, core_users + 1):
        for j in range(i + 1, min(i + 4, core_users + 1)):  # 每个用户连接后续3人
